...
Control Authority Protocol（CAP）定義終端設備如何驗證 Fay（iFay 或 coFay）已獲得其人類原型（Human Prime）授權，從而合法存取終端資源。協議以離線授權描述檔案（Authorization_Descriptor）為核心機制，線上可信票據（Trusted_Ticket）為補充，涵蓋授權校驗、會話管理、控制權交接、資源存取模式和存活檢測等核心能力，為 iFay 體系中智慧代理安全接管終端軟硬體提供標準化的控制權限框架。
